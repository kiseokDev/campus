...
diff_match_patch.prototype.diff_bisect_ = function (a, b, c) {
  ...
  for (
    ...
    for (var v = -t + p; v <= t - x; v += 2) {
      var n = f + v;
      var r =
        v == -t || (v != t && h[n - 1] < h[n + 1]) ? h[n + 1] : h[n - 1] + 1;
    ...
    for (v = -t + w; v <= t - q; v += 2) {
      n = f + v;
      u = v == -t || (v != t && l[n - 1] < l[n + 1]) ? l[n + 1] : l[n - 1] + 1;
      for (
        r = u - v;
...
diff_match_patch.prototype.diff_cleanupSemantic = function (a) {
  for (
    ...
    a[f][0] == DIFF_EQUAL
      ...
      : (a[f][0] == DIFF_INSERT ? (l += a[f][1].length) : (k += a[f][1].length),
        e &&
          e.length <= Math.max(g, h) &&
          e.length <= Math.max(l, k) &&
          (a.splice(c[d - 1], 0, new diff_match_patch.Diff(DIFF_DELETE, e)),
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a) {
  ...
  for (var c = 1; c < a.length - 1; ) {
    if (a[c - 1][0] == DIFF_EQUAL && a[c + 1][0] == DIFF_EQUAL) {
      ...
      g = d;
      h = e;
      for (var l = f, k = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0); ) {
        d += e.charAt(0);
...
diff_match_patch.prototype.diff_cleanupEfficiency = function (a) {
  for (
    var b = !1, c = [], d = 0, e = null, f = 0, g = !1, h = !1, l = !1, k = !1;
    f < a.length;
      ...
      case DIFF_EQUAL:
        1 < c + d
          ? (0 !== c &&
              ...
              0 !== g &&
                (0 < b - c - d && a[b - c - d - 1][0] == DIFF_EQUAL
                  ...
                  : (a.splice(
                      0,
                      0,
                      new diff_match_patch.Diff(DIFF_EQUAL, f.substring(0, g))
                    ),
                    b++),
                (f = f.substring(g)),
                (e = e.substring(g))),
            ...
            a.splice(b, c + d),
            e.length &&
              (a.splice(b, 0, new diff_match_patch.Diff(DIFF_DELETE, e)), b++),
            f.length &&
              (a.splice(b, 0, new diff_match_patch.Diff(DIFF_INSERT, f)), b++),
            b++)
...
diff_match_patch.prototype.match_bitap_ = function (a, b, c) {
  ...
  for (var k, m, p = b.length + a.length, x, w = 0; w < b.length; w++) {
    k = 0;
    for (m = p; k < m; )
      d(w, c + m) <= g ? (k = m) : (p = m), (m = Math.floor((p - k) / 2 + k));
    p = m;
    k = Math.max(1, c - m + 1);
    ...
    if (!(a[c].length1 <= b)) {
      ...
      for (var e = d.start1, f = d.start2, g = ""; 0 !== d.diffs.length; ) {
        ...
        "" !== k &&
          ((h.length1 += k.length),
          (h.length2 += k.length),
          0 !== h.diffs.length && h.diffs[h.diffs.length - 1][0] === DIFF_EQUAL
...
diff_match_patch.prototype.patch_fromText = function (a) {
  ...
  for (var c = 0, d = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@$/; c < a.length; ) {
    ...
    for (c++; c < a.length; ) {
      ...
      if ("-" == e) f.diffs.push(new diff_match_patch.Diff(DIFF_DELETE, g));
      else if ("+" == e)
        f.diffs.push(new diff_match_patch.Diff(DIFF_INSERT, g));
      else if (" " == e) f.diffs.push(new diff_match_patch.Diff(DIFF_EQUAL, g));
      else if ("@" == e) break;
